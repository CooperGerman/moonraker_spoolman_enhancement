...
class SpoolManager:
    ...
    # async def get_machine_mapping(self, silent) -> List[Dict[str, Any]]:
    #     '''
    #     Get all spools assigned to the current machine from spoolman db
    #     '''
    #     verbose = self.server.is_verbose_enabled()
    #     machine_hostname = self.server.get_host_info()["hostname"]
    #     logging.info(f"Getting spools for machine: {machine_hostname}")
    #     try:
    #         spools = []
    #         reponse = await self.http_client.get(
    #             url=f'{self.spoolman_url}/v1/spool',
    #         )
    ...
    #     except Exception as e:
    #         if verbose:
    #             logging.exception(f"Failed to retrieve spools mapped to machine {machine_hostname} from spoolman")
    #         self.server.add_log_rollover_item(
    #             "spoolman_get_machine_mapping", f"Failed to Connect to spoolman: {e}",
    #             not verbose
    #         )
    #     if spools:
    #         if not silent:
    #             await self.klippy_apis.run_gcode(f"M118 {len(spools)} spools found for this machine:")
    #             for spool in spools:
    #                 await self.klippy_apis.run_gcode(f"M118 {CONSOLE_TAB}{i} : {spool['filament']['name']}")
    #     if not silent and not spools:
    #         await self.klippy_apis.run_gcode(f"M118 No spools assigned to machine: {machine_hostname}")
